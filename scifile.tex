...
For instance when \textit{A. Fleming}, coming back from its holidays in 1928, found its \textit{staphs} culture boxed contaminated by the mushroom \textit{Penicillium Notatum}, from which he was able to extract an antibiotic substance he called penicillin, it was a kind of serendipity process.
...
\begin{rem}
  In reality, there is a surjective function from Wikipedia articles to YAGO Nodes. This property is due to YAGO's construction.
\end{rem}
...
\begin{algorithm}[!h]
  ...
  \begin{algorithmic}
    \REQUIRE $wish \in \{specific , general\}$ , $N \in \mathbb{N}$ , $a_c \in A$
    \STATE $A^* \leftarrow \emptyset$
    \WHILE {$ Card(A^* \setminus A_{visited}) \leq N $}
    \IF {$wish = specific$}
    \STATE $ r \leftarrow geometric.random(0.5)$
    \STATE Ask YAGO a for a random son of depth $r+1$ from $a_c$
    \STATE $ A^* \leftarrow A^* \cup \{a_{YAGO}\}$
    \ELSE
    \STATE $ r \leftarrow geometric.random(0.5)$
    \STATE Ask YAGO a for a random father of depth $r+1$ from $a_c$
    \STATE $ A^*  \leftarrow A^* \cup \{a_{YAGO}\}$ 
    \ENDIF
    \ENDWHILE
    \RETURN $A^* \setminus A_{visited}$
  \end{algorithmic}
\end{algorithm}


\newpage
In this algorithm, when we ``Ask YAGO a for a random son of depth $r+1$ from $a_c$'', it means that we pick a random article in YAGO ontology that follows a random walk of length $r+1$ begining on the state $a_c$. In ``Ask YAGO a for a random father of depth $r+1$ from $a_c$'', it is the same random variable but in reversed edges YAGO.
